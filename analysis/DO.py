...
def encrypt_DO(data, input_group):
    global A
    ...
    # for graph 2
    access_condition = input_group[0]
    dynamic_list = input_group[1]
    msg = input_group[2]
    # for graph 3
    # access_condition = input_group[3]
    # dynamic_list = input_group[4]
    ...
    generate_B()    
    encrypt(A)
    # start_time = re_encrypt({'q_do':do_key.public_key,'c0':c0, 'c1':c1, 'c2':c2, 'ct':ct,'ct_h':ct_h,'lamb':lamb,'ome':ome,'sigma1':sigma1,'sigma2':sigma2,'A':A,'p':p})
...
def encrypt (data):
    ...
    return enc_data
    # Use the shared key (k) for encryption/decryption
